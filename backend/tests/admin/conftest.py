"""Shared fixtures for `backend/tests/admin/`.

Duplicates (rather than imports) the small `_EphemeralPostgres` helper
already established in `tests/workspace/conftest.py` and
`tests/migrations/test_0007.py` — a real, throwaway PostgreSQL server
with the full schema (`alembic upgrade head`, including migrations `0008`
and `0009` this Unit adds) applied, matching this repo's established
"each PR's own test infrastructure stays self-contained" convention (see
`sdd/phase-1-workspace-users-accounts/apply-progress`, PR1 judgment call
3, and `tests/workspace/conftest.py`'s own docstring).
"""
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-admin-test-")
...
def admin_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(admin_db_sessionmaker: sessionmaker):
...
@pytest.fixture
def grant_platform_admin(db_session) -> Callable[..., None]:
    def _grant(*, user_id: uuid.UUID, granted_by: uuid.UUID | None = None) -> None:
        db_session.execute(
            sa.text(
                "INSERT INTO app.platform_admin (user_id, granted_at, granted_by_user_id) "
                "VALUES (:user_id, now(), :granted_by)"
            ),
...
@pytest.fixture
def app_factory(admin_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    def _make():
        app = create_app()
