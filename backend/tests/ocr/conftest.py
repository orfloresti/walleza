"""Shared fixtures for `app/ocr` + `app/ocr_worker` tests.

Duplicates the same `_EphemeralPostgres` harness already established in
`tests/transactions/conftest.py` (itself following
`tests/migrations/test_0002.py`'s precedent) — self-contained per test
area, matching Phase 1+'s established convention.
"""
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-ocr-test-")
...
def ocr_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(ocr_db_sessionmaker: sessionmaker):
    session = ocr_db_sessionmaker()
    try:
        yield session
        session.commit()
    finally:
...
@pytest.fixture
def app_factory(ocr_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    def _make():
        app = create_app()
