...
Duplicates the small `_EphemeralPostgres` helper already established in
`tests/workspace/conftest.py`/`tests/admin/conftest.py` — each PR's own
test infrastructure stays self-contained (see
`sdd/phase-1-workspace-users-accounts/apply-progress`, PR1 judgment call
3). This module needs BOTH workspace-membership and platform-admin
fixtures, since audit-log scenarios span owner actions and platform-admin
actions.
"""
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-audit-test-")
...
def audit_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(audit_db_sessionmaker: sessionmaker):
...
@pytest.fixture
def app_factory(audit_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    def _make():
        app = create_app()
