"""Shared fixtures for `app/recurring` tests.
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-recurring-test-")
...
def recurring_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(recurring_db_sessionmaker: sessionmaker):
    ...
    session = recurring_db_sessionmaker()
    try:
        yield session
        session.commit()
    finally:
...
@pytest.fixture
def app_factory(recurring_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    ...
    SAME real ephemeral Postgres `recurring_db_sessionmaker` uses — so
