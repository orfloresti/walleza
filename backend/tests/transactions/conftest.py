"""Shared fixtures for `app/transactions` tests.

Duplicates the same `_EphemeralPostgres` harness already established in
`tests/migrations/test_0002.py`, `tests/accounts/conftest.py`, and
`tests/categories/conftest.py` — self-contained per test area, matching
Phase 1/PR1's established precedent.
"""
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-transactions-test-")
...
def transactions_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(transactions_db_sessionmaker: sessionmaker):
    ...
    session = transactions_db_sessionmaker()
    try:
        yield session
        session.commit()
    finally:
...
@pytest.fixture
def app_factory(transactions_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    ...
    SAME real ephemeral Postgres `transactions_db_sessionmaker` uses — so
