"""Shared fixtures for `app/budgets` tests.

Duplicates the same `_EphemeralPostgres` harness already established in
`tests/migrations/test_0002.py`, `tests/categories/conftest.py`, and every
other per-feature test area — self-contained per test area, matching
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-budgets-test-")
...
def budgets_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(budgets_db_sessionmaker: sessionmaker):
...
@pytest.fixture
def app_factory(budgets_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    def _make():
        app = create_app()
