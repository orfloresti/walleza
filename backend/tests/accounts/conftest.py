"""Shared fixtures for `app/accounts` tests.

Duplicates the same `_EphemeralPostgres` harness already established in
`tests/migrations/test_0002.py`, `tests/auth/conftest.py`, and
`tests/workspace/conftest.py` — self-contained per test area, matching
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-accounts-test-")
...
def accounts_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(accounts_db_sessionmaker: sessionmaker):
    ...
    override — exactly matching `tests/workspace/conftest.py`'s pattern."""
    session = accounts_db_sessionmaker()
    try:
        yield session
        session.commit()
    finally:
...
@pytest.fixture
def app_factory(accounts_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    ...
    SAME real ephemeral Postgres `accounts_db_sessionmaker` uses — so
