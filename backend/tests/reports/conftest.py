"""Shared fixtures for `app/reports` tests.

Duplicates the same `_EphemeralPostgres` harness already established in
`tests/budgets/conftest.py` and every other per-feature test area —
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-reports-test-")
...
def reports_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(reports_db_sessionmaker: sessionmaker):
    session = reports_db_sessionmaker()
    try:
        yield session
        session.commit()
    finally:
...
@pytest.fixture
def app_factory(reports_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    def _make():
        app = create_app()
