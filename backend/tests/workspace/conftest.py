...
`workspace_db_sessionmaker` provisions a REAL, ephemeral PostgreSQL server
(not a mock, not SQLite) with `0002`'s schema applied (`alembic upgrade
head`), following exactly the same real-Postgres pattern already
established in `tests/migrations/test_0002.py` and `tests/auth/conftest.py`
— membership, invite-token, and get-or-create-race correctness must be
proven against real FK/unique constraints and real transactions, not an
in-memory stand-in.

This intentionally duplicates (rather than imports) the small
`_EphemeralPostgres` helper from those other modules: each PR's own test
infrastructure stays self-contained, matching PR1's established precedent
(see `sdd/phase-1-workspace-users-accounts/apply-progress`, PR1 judgment
call 3).
"""
...
class _EphemeralPostgres:
    ...
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-workspace-test-")
...
def workspace_db_sessionmaker() -> Iterator[sessionmaker]:
    """A `sessionmaker` bound to a real, ephemeral Postgres with `0002`
    (workspace/workspace_member/workspace_invite/account) already applied
...
@pytest.fixture
def db_session(workspace_db_sessionmaker: sessionmaker):
    ...
    override, exactly matching how a real request would behave."""
...
def seed_user(db_session) -> Callable[..., uuid.UUID]:
    ...
    minimal `app.app_user` row directly (this PR does not touch signup —
...
@pytest.fixture
def app_factory(workspace_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    ...
    SAME real ephemeral Postgres `workspace_db_sessionmaker` uses — so
