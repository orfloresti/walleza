"""Shared fixtures for `app/transfers` tests.
...
`tests/categories/conftest.py`, and `tests/transactions/conftest.py` —
self-contained per test area, matching Phase 1/2's established
precedent.
"""
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-transfers-test-")
...
def transfers_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(transfers_db_sessionmaker: sessionmaker):
    ...
    override — exactly matching `tests/transactions/conftest.py`'s
    pattern."""
    session = transfers_db_sessionmaker()
    try:
        yield session
        session.commit()
    finally:
...
@pytest.fixture
def app_factory(transfers_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    ...
    SAME real ephemeral Postgres `transfers_db_sessionmaker` uses — so
