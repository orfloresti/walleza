"""Shared fixtures for `app/templates` tests.

Duplicates the same `_EphemeralPostgres` harness already established in
`tests/migrations/test_0002.py`, `tests/accounts/conftest.py`,
`tests/categories/conftest.py`, `tests/transactions/conftest.py`, and
`tests/transfers/conftest.py` — self-contained per test area, matching
Phase 1/2/3's established precedent.
"""
...
class _EphemeralPostgres:
    def __init__(self, bin_dir: str) -> None:
        ...
        self.data_dir = tempfile.mkdtemp(prefix="walleza-pg-templates-test-")
...
def templates_db_sessionmaker() -> Iterator[sessionmaker]:
...
@pytest.fixture
def db_session(templates_db_sessionmaker: sessionmaker):
    ...
    override — exactly matching `tests/transfers/conftest.py`'s pattern."""
    session = templates_db_sessionmaker()
    try:
        yield session
        session.commit()
    finally:
...
def seed_user(db_session) -> Callable[..., uuid.UUID]:
    ...
    minimal `app.app_user` row directly and returns its id."""
...
@pytest.fixture
def app_factory(templates_db_sessionmaker: sessionmaker) -> Callable[[], object]:
    ...
    SAME real ephemeral Postgres `templates_db_sessionmaker` uses — so
